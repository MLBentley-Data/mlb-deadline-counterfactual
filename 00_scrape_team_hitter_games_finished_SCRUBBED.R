...
# ==============================================================================
# TEAM HITTER GAME SCRAPER
# Iteratively extracts team-level batting game logs season-by-season.
...
# --- BASE URL ---
BASE_URL <- "https://www.sports-reference.com/stathead/baseball/team-batting-game-finder.cgi"
...
for (yr in 2005:2025) {
  ...
  if (length(all_years) > 0) {
    ...
    write.csv(checkpoint, "data/stathead_team_batting_checkpoint.csv", row.names = FALSE)
  }
  
  Sys.sleep(8)  # Seasonal boundary break
}
...
if (length(all_years) > 0) {
  ...
  cat(sprintf("\nTotal rows: %d\n", nrow(final_df)))
  write.csv(final_df, "data/stathead_team_batting.csv", row.names = FALSE)
  cat("Saved to data/stathead_team_batting.csv\n")
} else {
