...
* **Runs Scored Model:** Outperformed the baseline in **53.7%** of games (Paired t-test: $t = -1.968, p = 0.0246$).
* **Runs Allowed Model:** Outperformed the baseline in **52.4%** of games (Paired t-test: $t = -3.120, p = 0.0009$).
* **Win/Loss Classifier:** Achieved **57.96% accuracy** (compared to the baseline's 54.70%), yielding an incredibly significant **McNemar's test** result ($\chi^2 = 7.307, p = 0.0069$).
...
![2025 Deadline Impact Summary](visuals/2025_deadline_impact_summary.jpg)
...
The Mets took an aggressive buying approach, bringing in arms like Ryan Helsley and Tyler Rogers along with outfielder Cedric Mullins. 
* **The Story:** The bats responded to the reinforcements, surging out of their historical peer interval to score **+0.89 runs/game above projection**. 
* **The Downfall:** This offensive explosion was entirely neutralized by a severe pitching collapse, with the staff surrendering **+0.89 runs/game more than projected**, costing the Mets 8 wins relative to their pre-deadline trajectory.

![Mets 2025 ITS Panel](visuals/team_deep_dives/NYM_2025_its_panel_FINAL.jpg)

### Case Study 2: Detroit Tigers (Actual: 87 Wins | Counterfactual Projection: 93.5 Wins | Impact: -7 Wins)
Detroit attempted to lock down a playoff push by trading for veteran arms Charlie Morton and Kyle Finnegan. 
* **The Story:** Despite the acquisitions, the counterfactual model shows the Tigers regressed severely from their blistering first-half pace. The offense went cold, dropping **-0.62 runs/game below projection**, leading to an overall underperformance of -7 wins against their expected baseline.

![Detroit 2025 ITS Panel](visuals/team_deep_dives/DET_2025_its_panel_FINAL.jpg)

---

## Reproduction & Dependencies

> **Data Dependency Notice:** To respect data provider Terms of Service, this repository does **not** host raw StatHead database tables. A personal subscription to StatHead is required to pull the game logs and populate the `/data` folder schemas before executing the scripts.
